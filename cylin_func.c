...
static double	dest_to_cylin(t_ray ray, t_cylinder *cyl)
{
	...
	v = sub_vectors(ray.origin, cyl->point);
	t[0] = scalar_dob(ray.direct, cyl->direct);
	t[1] = scalar_dob(v, cyl->direct);
	a_b_c[0] = scalar_dob(ray.direct, ray.direct) - (t[0] * t[0]);
	a_b_c[1] = 2 * (scalar_dob(ray.direct, v) - t[0] * t[1]);
	a_b_c[2] = scalar_dob(v, v) - (t[1] * t[1]) - cyl->radius * cyl->radius;
...
t_shape *sh)
{
	...
	if (sh)
	{
		...
		0));
		a = scalar_dob((*ray).direct, cyl->direct) * ((*ray).dest) +
		scalar_dob(nor, cyl->direct);
		nor = sub_vectors(nor, cyl->point);
		nor = v_to_len(sub_vectors(nor, v_to_len(cyl->direct, a, 0)), 1, 0);
